...
`docker compose run --rm migrate` creates the tables, constraints and the `active_promotions` view and seeds the `type = 'ingestion'` pricing rules of migration `0001`; there is no separate seed step, and no `DATABASE_URL` to get right by hand — the service composes it from the same `POSTGRES_*` variables the `postgres` service reads. It is safe to re-run: `drizzle-kit migrate` applies only what `__drizzle_migrations` does not already record. Expect about ten seconds each time — `--rm` throws the container's `node_modules` away with the container, so every run reinstalls.

Migrating is a second verb rather than part of `up` because `--wait` cannot express "this one finished" — it waits for a container to be running, or healthy where a healthcheck exists, and a one-shot is neither for long. Measured on Compose v5.5.0, it gets a one-shot wrong in both directions. One that has already exited counts as a wait failure, so a fast migration inside `up` makes a completely healthy stack exit 1. One still running satisfies it, so a real migration (about ten seconds) makes `up -d --wait` exit 0 while the schema is still being created — and exit 0 even when that migration is about to fail on its credentials. The second form is the dangerous one: a green `--wait` over an empty database is exactly what `--wait` is here to prevent. `run --rm` instead starts the dependencies, streams the migration's log, and returns the migration's own exit status — 0 when it succeeds and the tool's code when it does not — so a script can branch on it. The service sits behind the `migrate` profile so a plain `up` does not start it; `run` enables that profile itself. All of this is a stopgap: once the `api` image lands (issue #19) the migration moves into that image's entrypoint and the one-shot goes, and the two must never both run migrations.
