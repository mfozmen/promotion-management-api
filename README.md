...
- **A 4xx explains itself; a 5xx does not.** A client error carries a message written for the caller. A server error never returns the message its handler wrote — that goes to the log. Its **status** survives whenever it is a real 5xx, because `502` and `504` tell an operator something `500` does not; a status the response cannot carry (not an integer 4xx or 5xx, e.g. `1000` or `404.5`) collapses to `500 INTERNAL`. Its **code and message** survive only where the API owns public words for that code. So `503 READ_MODEL_NOT_READY` answers `503` with `"The read model is not ready yet; retry shortly"`; `502 INTERNAL` answers `502` with `"Internal server error"`; and `503 CONFLICT` answers `503` with `INTERNAL`, because a client branching on `CONFLICT` must never see it on a read-model outage (PR #30, commits `af38e0c` and `4f10c7f`). An unexpected error is returned as `500 INTERNAL` only — no internal detail reaches the client — and is logged under an `error` key as `{ type, message, stack, code }`, taken from the driver error underneath so no SQL text or bound parameter reaches the log either (ADR-0009).
