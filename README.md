...
- **What an operator's line contains.** An error logged under `err` is a fixed set of fields, not whatever the error happened to carry: `type`, a message, the stack's frames, and for a database fault the SQLSTATE as `code` with the `constraint` name. SQL text and bound values never reach a line — a driver message is replaced by `database error`, and the stack keeps its frames without its message lines — so diagnosing a failed statement means reading the SQLSTATE, the constraint and the call site rather than the query (ADR-0010, REVIEW.md 8.4).
