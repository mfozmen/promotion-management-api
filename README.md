...
Migrating is a second verb rather than part of `up` because `--wait` cannot express "this one finished" — it waits for a container to be running, or healthy where a healthcheck exists, and a one-shot migration is neither for long. It therefore gets a one-shot wrong in both directions: one still installing satisfies `--wait`, so a single-verb boot reports green over an empty schema, and one that has already exited counts as a wait failure, so a completely healthy stack exits 1. `run --rm` instead starts the dependencies, streams the migration's log, and returns the migration's own exit status — 0 when it succeeds and the tool's code when it does not — so a script can branch on it. The service sits behind the `migrate` profile so a plain `up` does not start it; `run` enables that profile itself. ADR-0003 holds the measurements behind this, the Compose version they were taken on, and the plan to fold the migration into the `api` image (issue #19).
