...
- **A 4xx explains itself; a 5xx does not.** A client error carries a message written for the caller. A server error never returns the message its handler wrote — that goes to the log. Its **status** survives whenever it is a real 5xx, because `502` and `504` tell an operator something `500` does not; a status the response cannot carry (not an integer 4xx or 5xx, e.g. `1000` or `404.5`) collapses to `500 INTERNAL`. Its **code and message** survive only as a pair: the status and the code together have to be one the API wrote public words for, and any other combination answers `INTERNAL` under the status it came with. So `503 READ_MODEL_NOT_READY` answers `503` with `"The read model is not ready yet; retry shortly"`; `502 INTERNAL` answers `502` with `"Internal server error"`; `503 CONFLICT` answers `503` with `INTERNAL`, because a client branching on `CONFLICT` must never see it on a read-model outage; and `500 READ_MODEL_NOT_READY` answers `500 INTERNAL`, because a retry code on a genuine fault would have the client retry forever (PR #30, commits `af38e0c`, `4f10c7f` and `a218bf2`). An unexpected error is returned as `500 INTERNAL` only — no internal detail reaches the client — and is logged under an `error` key as `{ type, message, stack, code }`, taken from the driver error underneath so no SQL text or bound parameter reaches the log either (ADR-0009).
...
- **A rejection names where, never what.** `details` is a list of `{ path, message }`, capped at the first 20 issues so a large body cannot amplify into a huge response — the path is generated by the API — `body`, `query`, `body.window` for a nested object, `body.items[3].sku` for an element of an array — and the client needs it; the rejected keys and values are the client's own input and are never reproduced in the response, so unknown fields are reported as a count at their location. The key names — never their values, the first 20 of them, each cut to 64 characters, with the full count alongside — are logged at `warn` with the correlation id, so an operator can still answer what was sent; `warn` because the root logger runs at pino's default `info`, where a `debug` line would never be written (ADR-0008, PR #30, commits `de3bf9e`, `af38e0c`, `4f10c7f` and `a218bf2`).
