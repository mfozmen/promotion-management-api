...
`docker compose run --rm migrate` creates the tables, constraints and the `active_promotions` view and seeds the `type = 'ingestion'` pricing rules of migration `0001`; there is no separate seed step, and no `DATABASE_URL` to get right by hand — the service composes it from the same `POSTGRES_*` variables the `postgres` service reads. It is safe to re-run: `drizzle-kit migrate` applies only what `__drizzle_migrations` does not already record.

Migrating is a second verb rather than part of `up` because `--wait` cannot express "this one finished". A one-shot that exits — even successfully — counts as a wait failure, so with the migration inside `up`, `docker compose up -d --wait` exits 1 on a completely healthy stack (measured on Compose v5.5.0: `container …-migrate-1 exited (0)`, shell status 1). That made the first command in this guide report failure on success, and made a real failure indistinguishable from it. `run --rm` instead starts the dependencies, streams the migration's log, and returns the migration's own exit status — 0 when it succeeds and the tool's code when it does not — so a script can branch on it. The service sits behind the `migrate` profile so a plain `up` does not start it; `run` enables that profile itself. All of this is a stopgap: once the `api` image lands (issue #19) the migration moves into that image's entrypoint and the one-shot goes, and the two must never both run migrations.
