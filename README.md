...
| GET    | `/api/admin/queues/stats`    | Per queue: `waiting`, `active`, `delayed`, `failed` and the age of the oldest waiting job                               | `200`, `500`        |

`/api/admin/queues/stats` is the operator's read-only view of the four queues, and it is the only thing in the tree that can see the dead-letter set: `removeOnFail: false` keeps every exhausted job in BullMQ's failed set. `oldestWaitingAgeSeconds` is `null` when the read returned no waiting job, which is a different fact from an age of zero; it is read separately from `waiting`, so the two can disagree by one poll. The reporter is a required dependency of `createApp`, so the route is always served by a running process: there is no wiring in which it answers about queues it cannot see.
