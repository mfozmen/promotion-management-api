...
- **A 4xx explains itself; a 5xx does not.** A client error carries a message written for the caller. A server error never returns the message its handler wrote — that goes to the log — and answers with its status, its code, and a public message the API owns: `503 READ_MODEL_NOT_READY` reads `"The read model is not ready yet; retry shortly"`, and a 5xx code with no public message falls back to `"Internal server error"`. A code survives only under a status that is a real 5xx, so a mismatched one (`HttpError(1000, 'CONFLICT')`) answers `500 INTERNAL` rather than inviting a client to branch on a conflict that is a server fault (PR #30, commit `af38e0c`). An unexpected error is returned as `500 INTERNAL` only — no internal detail reaches the client — and is logged under an `error` key as `{ type, message, stack, code }`, taken from the driver error underneath so no SQL text or bound parameter reaches the log either (ADR-0009).
...
- **A rejection names where, never what.** `details` is a list of `{ path, message }` — the path is generated by the API (`body`, `query`, `body.items[3].sku`) and the client needs it; the rejected keys and values are the client's own input and are never reproduced in the response, so unknown fields are reported as a count at their location. The key names — never their values, and capped — are logged at `warn` with the correlation id, so an operator can still answer what was sent; `warn` because the root logger runs at pino's default `info`, where a `debug` line would never be written (ADR-0008, PR #30, commits `de3bf9e` and `af38e0c`).
