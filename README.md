...
It applies [`scripts/demo-seed.sql`](./scripts/demo-seed.sql) as a single transaction: 1 000 products over `Electronics`, `Apparel`, `Home` and `Sports`, and one seven-day 20 % flash sale on `Electronics`. Run it as often as you like — products upsert on `sku` and rewrite only a row whose values or provenance differ from what this run would leave, and the flash sale is deleted by name and re-inserted, because the `promotions_no_overlapping_active_category` exclusion constraint would reject a second active row over the same category and window. It is a script rather than a migration because a production database must be able to skip it; the `ingestion` pricing rules a vendor import applies are reference data, not demo data, and are seeded by migration `0001_seed_pricing_rules.sql`. What you get depends on the migrations and this run alone, never on what a previous run left. The sale's window opens at the moment of the run, so re-running is also how a demo database left for more than a week gets a live sale back. A row an import had claimed goes back to being a demo row, provenance columns and all. Two seeds at once are safe rather than forbidden: they serialise on the product rows, and whichever commits second deletes the first's sale by name before writing its own, so you still get one catalogue and one sale. What the seed will not do is replace a promotion it does not own — an active `Electronics` promotion under another name is not deleted by name, so the insert aborts on `23P01` and the whole file rolls back, leaving no half-written catalogue behind.
