...
- **Request bodies** are capped at 100kb. Everything body-parser refuses keeps the status that says which failure it was — 400 for a body that cannot be read, 413 for one over the cap, 415 for a charset it will not decode — and answers `BAD_REQUEST` with one message of ours. The status carries the distinction, so the code need not; the parser's own messages quote the input back and never cross (ADR-0009).
