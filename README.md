...
| GET    | `/api/admin/queues/stats` | Per queue: `waiting`, `active`, `delayed`, `failed` and the age of the oldest waiting job | none                                                                                                                                                                             |

`/api/admin/queues/stats` is the operator's read-only view of the four queues, and it is the only thing in the tree that can see the dead-letter set: `removeOnFail: false` keeps every exhausted job in BullMQ's failed set. `oldestWaitingAgeSeconds` is `null` when the read returned no waiting job, which is a different fact from an age of zero; it is read separately from `waiting`, so the two can disagree by one poll. `src/server.ts` builds the app with the reporter, so the route is served; an app built without one has no such route at all, rather than answering about queues it cannot see.
