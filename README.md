...
Migrating is not a step anyone has to remember: the `migrate` one-shot service applies `src/shared/db/migrations/` as part of `up`, and `--wait` does not return until it has exited cleanly. That single step creates the tables, constraints and the `active_promotions` view and seeds the `type = 'ingestion'` pricing rules of migration `0001`; there is no separate seed step. A failed migration is not retried — the service carries `restart: 'no'` — so `up -d --wait` exits non-zero with PostgreSQL and Redis running and the schema partially applied. Read the cause with `docker compose logs migrate`, fix it, and re-run the migration with `docker compose up migrate`, which starts that one service and streams its log to the terminal instead of re-waiting on the whole stack. A second `up -d --wait` re-runs it too — `restart: 'no'` only stops the daemon retrying the exited container, it does not stop `up` starting it again — it just reports the failure as a stack-wide `--wait` error. Neither verb hands back the migration's exit status: attached `up migrate` returns 0 against a migration that exited 3, `up -d --wait` returns 1, so a script that branches on the real code needs `docker compose up --exit-code-from migrate migrate` (ADR-0003). It is a stopgap — once the `api` image lands (issue #19) the migration moves into that image's entrypoint and the one-shot service goes, so the two never both run migrations.
