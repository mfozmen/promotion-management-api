...
| ------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **The database schema DDL**           | [`docs/schema.sql`](./docs/schema.sql) - the whole schema as one file, taken with `pg_dump` from a database created empty and migrated forward. The migrations that produce it are [`src/shared/db/migrations/`](./src/shared/db/migrations), and they are the input; the file is a copy kept for a reader who wants to open one thing — a copy an integration test keeps honest, by building one database from the migrations and one from this file and comparing what the PostgreSQL catalog reports for each. |
