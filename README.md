...
- **A 4xx explains itself; a 5xx does not.** A client error carries a message written for the caller. A server error returns its status and its code and a generic `"Internal server error"` message whatever raised it, so `503 READ_MODEL_NOT_READY` stays something a client can branch on while its prose stays server-side. An unexpected error is returned as `500 INTERNAL` only — no internal detail reaches the client — and is logged under an `error` key as `{ type, message, stack, code }`, taken from the driver error underneath so no SQL text or bound parameter reaches the log either (ADR-0009).
...
- **A rejection names where, never what.** `details` is a list of `{ path, message }` — the path is generated by the API (`body`, `query`, `body.items[3].sku`) and the client needs it; the rejected keys and values are the client's own input and are never reproduced in the response, so unknown fields are reported as a count at their location. The keys are logged at `debug` with the correlation id, so an operator can still answer what was sent (ADR-0008, PR #30, commit `de3bf9e`).
