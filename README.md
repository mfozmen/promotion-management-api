...
compose server's role). CI's "No schema drift" step regenerates and runs
`git diff --exit-code src/shared/db/migrations`, so a schema change committed without its
migration fails the build (commit `ba5c2ca`).
