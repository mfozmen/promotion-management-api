...
It is a rule and not a check on purpose. The free plan's quality gate judges ratings, coverage, duplication and hotspot review, so a CRITICAL code smell passes it, and a gate condition on issue count is not available on this project's plan (the owner established this on PR #56; it is the constraint the rule exists under, not something re-derived here). PR #56 built a CI step that queried SonarCloud's issue API instead, reviewed it twice, then deleted it: it restated what the pull request comment already says. Read that entry in `docs/ai-appendix-notes.md` before rebuilding it.
