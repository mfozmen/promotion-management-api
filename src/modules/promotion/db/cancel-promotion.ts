 ...
 */
export async function cancelPromotion(db: Db, id: number): Promise<PromotionWriteOutcome> {
  ...
  if (cancelled) return { ok: true, promotion: cancelled };
  ...
  const [already] = await db
    ...
    .limit(1);
  return already ? { ok: true, promotion: already } : { ok: false, reason: 'not-found' };
}
