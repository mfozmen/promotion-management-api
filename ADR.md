...
- **No test asserts the seeded production default, and that is what makes "the policy is data" true rather than decorative.** A test that pinned product-level precedence, or lowest-price, would be asserting a configuration value: the row would be unchangeable without turning CI red, and a policy that cannot change is not data. Tests assert the mechanism instead — given a rule row the test itself inserts, the engine selects that candidate — so the seeded default can be edited in production without a code change, which is the whole reason the layer exists.
- **The rule set is cached for 60 seconds and nothing invalidates it.** After a policy edit, workers hold two policies for up to a minute, and thereafter only products that receive an event are re-resolved; the rest keep the old price until something re-resolves them. What would: a `readmodel.rebuild` triggered on a `pricing_rules` write, or the reconciler's sweep widened to re-resolve on a rules-version change. Neither is built here. Stated so the first policy edit in production is not the thing that discovers it.
