...
- The clone-per-file scheme costs one `drop database` per file at teardown, and each forces a cluster-wide checkpoint and fsync (~620 ms measured, against ~37 ms to clone the template), so the integration project's `hookTimeout` is sized for the teardown rather than the setup. That is linear in the number of integration test files; past roughly a dozen the answer is `fsync=off` on a throwaway test server, not a longer timeout.
