...
- Migration `0000` is edited in place and regenerated whenever the schema changes, because nothing is deployed yet. `__drizzle_migrations` keys on the file hash, so a database that already ran an older `0000` would keep the older shape for ever: the forward-only rule (REVIEW.md 11.1) starts at the first deploy, and from then on a schema change is a new migration.
