...
- The first module that opens a Redis connection did **not** wire `loadConfig` in. `src/shared/queue.ts` holds the queue index as a `QUEUE_DB = 1` constant — the `REDIS_QUEUE_DB` default, duplicated rather than read — and `src/server.ts` reads `REDIS_URL`, `PORT` and `SHUTDOWN_TIMEOUT_MS` straight from `process.env` (PR #37, `829d6bb`). `SHUTDOWN_TIMEOUT_MS` is therefore a variable the application reads that appears in neither `config.ts` nor `.env.example`. Both are settled when the wiring lands with the first module that needs a pool (issue #10); until then the duplication sits in two files rather than one.
