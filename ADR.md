...
- The first module that opens a Redis connection did **not** wire `loadConfig` in. `src/shared/queue.ts` holds the queue index as a `QUEUE_DB = 1` constant — the `REDIS_QUEUE_DB` default, duplicated rather than read — and `src/server.ts` reads `REDIS_URL`, `PORT` and `SHUTDOWN_TIMEOUT_MS` straight from `process.env` (PR #37, `829d6bb`). `SHUTDOWN_TIMEOUT_MS` is listed in `.env.example` but is the one variable `config.ts` does not parse, so its default is duplicated too: `.env.example` carries 10 000 and `src/shared/shutdown.ts` hard-codes the value that actually runs when the variable is absent or malformed. Both duplications are settled when the wiring lands with the first module that needs a pool (issue #10).
