...
- **A 5xx never returns its message, whoever raised it.** The masking rule is not "errors we did not construct": an `HttpError` carrying a 5xx is a server fault whose message was written for an operator, so only its status and code cross — `503 READ_MODEL_NOT_READY` stays branchable by the client without its prose, and the message goes to the log at `error`. The guard that decides this is a bounded integer check: Express 5 throws a `RangeError` for a status outside `[100, 999]`, so an unbounded status turns the error handler itself into the failure, which is the HTML-error-page leak two earlier rounds closed. Client statuses are integers in `[400, 499]` and pass through whole. A raised error outside that range keeps its `code`, and its status only if that status is itself an integer 5xx; a status Express could not write at all — `42`, `1.5`, a string — becomes `500`, and the message is replaced either way (commit `de3bf9e`).
- **A masked 5xx drops its code with its status.** A code whose status does not match it is worse than no code: a client branching on `CONFLICT` under a `500` would never retry what is actually a server fault. Only a status that is a real 5xx keeps the code the handler chose, and the public message for it comes from a map keyed by `ErrorCode` — ours by construction rather than by trusting the raiser, so `503 READ_MODEL_NOT_READY` reads "The read model is not ready yet; retry shortly" rather than "Internal server error", which would tell a human the opposite of the truth. A handler's `details` never crosses on a 5xx either.
- **The rejected keys are logged at `warn`, not `debug`.** The root logger runs at pino's default `info`, so a `debug` line is a mitigation that never fires in production while passing every test — which is what the first version of this did. Field names only, never values, and capped, because the number of unknown keys is the client's to choose. An `HttpError` message must therefore never carry a credential: on a 5xx it goes to the log and not to the client, but it does go to the log.
- **A rejection names where, never what.** "Client input is not echoed back" now holds on all three paths, not two: `notFoundHandler` withholds the path, body-parser's messages are replaced, and zod's unrecognized-key message — which quotes the key the client sent — is replaced with the location and a count. The path (`body`, `window`, `body.items[3].sku`) is ours, we generate it, and the client needs it to fix the request; the key and the value are theirs. The rejected keys go to the log at `debug` with the correlation id, so an operator can still answer "what did they send".
...
- Peak RSS under load is a V8 heap question, not an application one: `e2e-tester` measured 233 MB at `-c 100` on the built artifact for a route that serialises `{"status":"ok"}`, returning to 59 MB afterwards. On a 256 MB container that is thin headroom before any database or Redis client exists, so the deployment sets `--max-old-space-size` below the container limit rather than trusting the default.
