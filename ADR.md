...
- Schema drift is a CI step rather than a convention: `npm run db:generate` then `git diff --exit-code` over `src/shared/db/migrations` fails the build when `src/shared/db/schema/` was edited without regenerating. That is the direction it catches. The opposite one — a regeneration that drops the four hand-written objects — it cannot catch, because a committed regeneration leaves a clean tree; the integration tests are what notices that, which is why they assert each of the four directly.
...
- Four SQL objects are hand-written into the migrations and have no expression in `src/shared/db/schema/`: the `btree_gist` extension, the two promotion exclusion constraints, the `pricing_rules_set_updated_at` trigger with its function, and the `reconciler_state` seed row. `npm run db:generate` cannot reproduce them, so a regeneration drops all four. It is no longer silent: CI's "No schema drift" step runs `db:generate` and `git diff --exit-code src/shared/db/migrations`, so both directions of drift — a schema file changed without regenerating, and a regeneration that dropped the four hand-written objects — fail the build on the diff, with the integration tests as the second line (`ba5c2ca`). The step needs no database; `generate` reads the schema files and the snapshots. The `active_promotions` view is not a fifth: `drizzle-kit` generated migration `0002` from the `pgView` in `src/shared/db/schema/active-promotions.ts`, so it is inside what a regeneration emits rather than outside it, and `npm run db:generate` reports no changes on a clean tree.
...
- **`products.pricing_rules_version` cannot see a deletion.** It stores the rule set's `max(updated_at)` in epoch milliseconds, so an operator deleting a rule leaves the version unchanged and two rows stamped with the same value may have been priced by different rule sets. It identifies the newest edit, not the rule set; a hash over the active rows would identify the set, and is not built here.
