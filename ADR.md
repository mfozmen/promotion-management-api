...
- **Unknown routes** hit a `notFoundHandler` before the error handler, so a typo gets JSON `404 NOT_FOUND` rather than Express's HTML page. The requested path is not echoed back, because the caller already knows the path it asked for, so echoing buys no diagnostic the log does not already have — not because it is untrusted, which a rejected key equally is.
