...
A third non-profile service, `migrate`, runs `npm ci --ignore-scripts && npx drizzle-kit migrate` in a `node:22-alpine` container behind `depends_on: postgres: service_healthy`, so `docker compose up -d --wait` leaves a migrated schema in place and `npm run db:migrate` is not a step anyone has to remember (PR #50, `ba5c2ca`; `--ignore-scripts` added in `888e632`). It carries `restart: 'no'`, which is what makes `--wait` treat its clean exit as completion rather than waiting for a healthcheck it does not have, and its `DATABASE_URL` is composed from the same `POSTGRES_*` variables the `postgres` service reads, so the two cannot drift apart in one `.env`. `restart: 'no'` also means a failed migration is not retried: `up -d --wait` exits non-zero with the stores running and the schema partially applied, and the recovery verb once the cause is fixed is `docker compose up migrate`, which starts that one service and streams its log where the failure can be read. Not because `up -d` would refuse — it starts the exited container again too, since `restart: 'no'` governs the daemon retrying a container rather than `up` starting one — but because it re-waits on the whole stack and surfaces a stack-wide `--wait` error instead of the migration's output. Neither verb propagates the migration's exit code, measured on a one-shot service that exits 3: attached `up migrate` exits 0, `up -d --wait` exits 1, and only `docker compose up --exit-code-from migrate migrate` exits 3. Nothing scripted should read a migration failure from `up` alone.
