...
**Status:** Accepted — issue #6, PR #30 (branch `feat/http-skeleton`, commit `7fd588e` plus the review-fix commits `e46f14b`, `c6e4ff2`, `ccef8a4`, `de3bf9e`, `af38e0c` and `4f10c7f`)
...
- **A 5xx keeps its status; its code and its words have to be earned.** A code whose status does not match it is worse than no code: a client branching on `CONFLICT` under a server fault would never retry. The status and the code are therefore decided separately. A status the response cannot carry — anything that is not an integer 4xx or 5xx, so `42`, `1.5`, `4004`, `1000`, and equally a 3xx no handler at this boundary raises — collapses to `500 INTERNAL`; a real 5xx keeps its status, because `502` and `504` are exactly what an operator needs and collapsing them to `500` deletes the upstream-failure signal. The code and the message then cross only where `SERVER_MESSAGES` has public words for that code, from a map keyed by `ErrorCode` — ours by construction rather than by trusting the raiser. So `503 READ_MODEL_NOT_READY` keeps both and reads "The read model is not ready yet; retry shortly" rather than "Internal server error", which would tell a human the opposite of the truth; `502 INTERNAL` keeps its status under the generic message; and `503 CONFLICT` keeps its status while the mismatched code is dropped to `INTERNAL`, so a client never sees `CONFLICT` on a read-model outage. A handler's `details` never crosses on a 5xx either. Gating the whole mapping on the message map was the regression this bullet corrects: it collapsed `502 INTERNAL` to `500` and lost the status with the words, and `e2e-tester` caught it black-box (commit `4f10c7f`; the map itself is `af38e0c`). `tests/error-handler.test.ts` pins the `502 INTERNAL` and `503 CONFLICT` cases on the status as well as on the body.
- **The rejected keys are logged at `warn`, not `debug`.** The root logger runs at pino's default `info`, so a `debug` line is a mitigation that never fires in production while passing every test — which is what the first version of this did. Field names only, never values, and bounded in count **and** in bytes — the first 20 keys, each cut to 64 characters, with the full count alongside — because both the number of unknown keys and their length are the client's to choose, and this line is written on an unauthenticated path: 20 keys of four kilobytes is an 80 kB line per request. The cap had been claimed here and pinned by no test until a case posted 25 keys of 200 characters and asserted both bounds (commit `4f10c7f`). An `HttpError` message must therefore never carry a credential: on a 5xx it goes to the log and not to the client, but it does go to the log. `tests/request-validator.test.ts` builds the logger at `info` so the mitigation cannot silently stop firing again (commit `af38e0c`).
- **A rejection names where, never what.** "Client input is not echoed back" now holds on all three paths, not two: `notFoundHandler` withholds the path, body-parser's messages are replaced, and zod's unrecognized-key message — which quotes the key the client sent — is replaced with the location and a count. The path (`body`, `query`, `body.window`, `body.items[3].sku`) is ours, we generate it, and the client needs it to fix the request; the key and the value are theirs. It is **built** from the part name and zod's path segments — an array index becomes `[3]`, a property becomes `.sku` — not joined with dots: joining emitted `items.3.sku`, which matched neither this ADR nor the README, for the whole life of the PR, because no test until `4f10c7f` asserted a nested path. The rejected keys go to the log at `warn` with the correlation id (see the bullet above for why not `debug`), so an operator can still answer "what did they send".
