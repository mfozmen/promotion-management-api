...
- **Stack logged, never returned.** An unexpected error is logged with `req.log.error({ error: serializeError(err) })` — stack frames, error type, message and SQLSTATE — and the client receives only `{ code: 'INTERNAL', message: 'Internal server error' }`. REVIEW.md §8.4 is amended in this PR to say so explicitly, as its own preamble requires when a rule and the design disagree: the response carries nothing internal, and the log carries the stack but never SQL text, bound parameters or secrets. The error is never handed to a logger as an object, because a driver error carries the failing statement and the request body on its own fields and in its message; the whitelist in ADR-0009 is what reaches the line.
...
- Delegating to Express after `res.headersSent` means its final handler writes the raw `err.stack` to stderr (`logerror` in `express/lib/application.js`, on every env except `test`), so a truncated response leaves a non-JSON block among the JSON lines — and the first line of a stack is the error's own message, which for a `DrizzleQueryError` is the statement and its bound parameters. This is the one path the ADR-0009 whitelist does not cover. It is reachable only after a partial write, and no route streams a response today; **how to close it (a terminal handler of our own, or destroying the socket in the error handler instead of calling `next`) is undecided** and belongs to the first streaming route, ADR-0006's listing.
