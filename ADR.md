...
**Status:** Accepted — issue #6, PR #30 (branch `feat/http-skeleton`, commit `7fd588e` plus the review-fix commits `e46f14b`, `c6e4ff2`, `ccef8a4`, `de3bf9e`, `af38e0c`, `4f10c7f` and `a218bf2`)
...
- **A 5xx never returns its message, whoever raised it.** The masking rule is not "errors we did not construct": an `HttpError` carrying a 5xx is a server fault whose message was written for an operator, so the raiser's message never crosses — it goes to the log at `error`, and what the client reads instead is a message of ours (next bullet). The guard that decides this is a bounded integer check: Express 5 throws a `RangeError` for a status outside `[100, 999]`, so an unbounded status turns the error handler itself into the failure, which is the HTML-error-page leak two earlier rounds closed. Client statuses are integers in `[400, 499]` and pass through whole. A raised error outside that range keeps its status only if that status is itself an integer 5xx; a status Express could not write at all — `42`, `1.5`, a string — becomes `500`, and the raiser's message is replaced either way (commit `de3bf9e`). What happens to the `code`, and which message takes its place, is the next bullet.
- **A 5xx keeps its status; its code and its words have to be earned.** A code whose status does not match it is worse than no code: a client branching on `CONFLICT` under a server fault would never retry. The status and the code are therefore decided separately. A status the response cannot carry — anything that is not an integer 4xx or 5xx, so `42`, `1.5`, `4004`, `1000`, and equally a 3xx no handler at this boundary raises — collapses to `500 INTERNAL`; a real 5xx keeps its status, because `502` and `504` are exactly what an operator needs and collapsing them to `500` deletes the upstream-failure signal. The code and the message then cross only as a **pair**, never one without the other: `SERVER_MESSAGES` is keyed by **status** and each entry carries the code that belongs with it, so a raised error keeps its code and gets public words only on an exact `(status, code)` match, and is `INTERNAL` under its own status otherwise (commit `a218bf2`). So `503 READ_MODEL_NOT_READY` — the one pair in the map today — keeps both and reads "The read model is not ready yet; retry shortly" rather than "Internal server error", which would tell a human the opposite of the truth; `502 INTERNAL` keeps its status under the generic message; `503 CONFLICT` keeps its status while the mismatched code is dropped to `INTERNAL`, so a client never sees `CONFLICT` on a read-model outage; and `500 READ_MODEL_NOT_READY` answers `500 INTERNAL`, because a real code under the wrong status would have a client retry a genuine fault forever. A handler's `details` never crosses on a 5xx either. The pair is checked structurally because asserting it in prose failed three times in three commits: the code survived a status it did not match (before `af38e0c`), then the fix dropped the status along with the code, which collapsed `502 INTERNAL` to `500` and deleted the upstream signal — `e2e-tester` caught that black-box (`4f10c7f`) — and then a map keyed by `ErrorCode` accepted a code under any 5xx at all (`a218bf2`). One map keyed by status is one place for the invariant instead of three. `tests/unit/error-handler.test.ts` pins `502 INTERNAL`, `503 CONFLICT` and `500 READ_MODEL_NOT_READY` on the status as well as on the body.
...
- **`HttpError` does not take a status.** `HttpError(code, message, details?)`, and `STATUS`, a `Record<ErrorCode, number>` exhaustive over the closed vocabulary, says what each code answers with. The pairing rule this replaces was wrong in three different directions across three consecutive commits — a code surviving a status it did not match, then the status dropped along with the code, then a code accepted under any 5xx at all. Each fix was right about what it aimed at and wrong about what stood next to it, because one decision was being restated in three places. It is now unstateable: `new HttpError(404, 'CONFLICT', …)` does not compile, both status-range guards inside the raise path are gone, and the REVIEW.md rule that would have asked authors not to write it is not needed. A rule cannot be broken by code that cannot express the violation. Body-parser's own errors are untouched by this: they are foreign, carry their own arbitrary 4xx, and never construct an `HttpError`.
- **The cost, stated rather than discovered:** `502` and `504` are no longer expressible, because no code in the vocabulary maps to them. That retires the "a 5xx keeps its status" behaviour two earlier rounds argued over. Adding one is a code and a row in `STATUS`, decided together, which is the point — the alternative was any handler being able to invent a status for any code.
- **A rejection names where, never what.** "Client input is not echoed back" now holds on all three paths, not two: `notFoundHandler` withholds the path, body-parser's messages are replaced, and zod's unrecognized-key message — which quotes the key the client sent — is replaced with the location and a count. The path (`body`, `query`, `body.window`, `body.items[3].sku`) is ours, we generate it, and the client needs it to fix the request; the key and the value are theirs. It is **built** from the part name and zod's path segments — an array index becomes `[3]`, a property becomes `.sku` — not joined with dots: joining emitted `items.3.sku`, which matched neither this ADR nor the README, for the whole life of the PR, because no test until `4f10c7f` asserted a nested path. `details` is **bounded at the first 20 issues**, for the same reason the log line is bounded and against the same input: one 100kb body of array items produces thousands of zod issues, so an unauthenticated request would otherwise amplify a small body into a several-hundred-kilobyte response. A client fixing 20 fields at a time is the cost (commit `a218bf2`). The rejected keys go to the log at `warn` with the correlation id (see the bullet above for why not `debug`), so an operator can still answer "what did they send".
...
- Peak RSS under load is a V8 heap question, not an application one, and it is not a single number: across three runs at `-c 100` `e2e-tester` measured 200, 203 and 257 MB on the built artifact for a route that serialises `{"status":"ok"}`, each returning to 59 MB afterwards. One of the three crossed the 256 MB container the design assumes. At the contracted `-c 50` the peak was 232 MB with no crossing. **Nothing caps the heap today**: there is no Dockerfile, `start` is a bare `node dist/server.js`, and no `--max-old-space-size` or `NODE_OPTIONS` is set anywhere in the repository. Setting it below the container limit is the mitigation and it belongs to the PR that adds the container, not to this one; until then the headroom is whatever V8 chooses. An earlier version of this bullet quoted a single 209 MB sample and asserted the flag as though it were already set, which is the same defect as a control proved only in a test configuration.
