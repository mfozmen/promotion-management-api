...
Which candidate applies is decided by `json-rules-engine` rules stored in `pricing_rules` (`type = 'promotion'`) and cached for 60 seconds, not by hard-coded precedence (owner decision, 2026-09-12). The resolver computes each candidate's discount first, then hands the engine a single fact object holding both candidates, so a rule can compare them; the highest-priority rule that fires names the winning level, ties breaking on the lower promotion id; the seeded default rule applies whichever candidate prices lower, in the customer's favour, so a category flash sale deeper than a product's own promotion wins. That default is not free to choose: REVIEW.md 7.4 makes it a required edge case and section 4 of the design spec states it with its consequence for admins. Changing the policy remains a row edit rather than a deploy.
...
The rule "at most one active promotion per product" is implemented as **at most one applied promotion**: whichever active candidate prices lower, ties breaking on the lower promotion id, else none. The arithmetic lives in that one function and nowhere else (REVIEW.md 1.3): percentage is `base - floor(base * bps / 10000)`, fixed is `max(base - value, 0)`, computed in `bigint` and clamped into `[0, base]`. Three mutations exist: create (with a target → `active`, or without → `draft`), assign (`draft` → `active`, target set in the same guarded `UPDATE`, which rejects with `409` when the row is not a draft, when a concurrent assign won, or when its `ends_at` has already passed, and runs the exclusion constraints in the same statement) and cancel. A draft is never applied. Which promotions are candidates is decided by the database clock, in the resolution query's `tstzrange(starts_at, ends_at) @> now()` predicate; `isActive` mirrors the same half-open window in TypeScript for a caller that already holds a row, and is not a second source of truth. Read endpoints for promotions serve the admin path from PostgreSQL. Boundaries are scheduled with delayed BullMQ jobs at `starts_at` and `ends_at`, enqueued at the moment a promotion first becomes active (create-with-target or assign), so an assigned draft reaches the read model immediately.
...
- Under the seeded default a category flash sale still reaches products that carry their own active promotion whenever it prices lower, because the deeper discount wins. The applied promotion is returned in every storefront response, so an admin can see which candidate won. A product that must not be discounted further is expressed as a higher-priority rule, not as code.
- The retired shape is unconstructible in the application. `src/modules/promotion/promotion.ts` declares `DiscountType` as `percentage | fixed` and a `Promotion` interface carrying `discountType`, `value`, `status`, `startsAt` and `endsAt` — no `calculator` field and no parameter bag to put one in, so a promotion naming a calculator class does not typecheck, and `applyPromotion` branches on the two literal types with no registry lookup to reach. The promotion write store has not landed on this branch: its DDL in the design spec carries `discount_type` and `value` only, so there is no `calculator` column and no `params` bag for a later migration to drop.
...
- Which campaign wins is now a function of `base_price_cents`, so a vendor feed can change the credited promotion with no admin action and no promotion mutation: a product-level fixed discount and a category percentage swap places when the base price moves far enough. Product-level precedence was stable under base-price changes; the lower price is not. The margin-floor exception rule is therefore the only protection for a deliberately priced product, and carries weight it did not carry before.
...
- Unconditional product-level precedence: a product's own promotion would beat a deeper category flash sale, so a customer would see a worse price than the campaign advertises. Rejected by the owner (2026-09-12) in favour of whichever candidate prices lower.
