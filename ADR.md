...
`domain/dto/` is the one directory named for what it holds rather than for a job. It holds every shape, not only type aliases: a zod schema is a shape that can check itself, so `vendor-row-facts.ts` and `adjustment-event.ts` are dto, and so is the payload of a queue message, defined once in the module that emits it and imported by the module that consumes it — the one direction in which a module may import from another. `domain/` itself is behaviour only. `adjustment-event` keeps the library's word for a rule's action; it is not a system event. With types and functions at one level a reader could not tell a data shape from the logic over it without opening the file; the split is taken for that reason and for nothing else — `http/`, `db/` and `jobs/` are not divided further. Promotion and pricing are separate modules that import nothing from each other (ADR-0004, ADR-0005).
