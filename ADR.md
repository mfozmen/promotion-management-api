...
- Schema drift is a CI step rather than a convention: `npm run db:generate < /dev/null`, then `git add -AN src/shared/db/migrations` and `git diff --exit-code src/shared/db/migrations`, fails the build when `src/shared/db/schema/` was edited without regenerating. That is the direction it catches. The opposite one — a regeneration that drops the four hand-written objects — it cannot catch, because a committed regeneration leaves a clean tree; the integration tests are what notices that, which is why they assert each of the four directly. Three extra pieces are load-bearing, and the first is the reason the step is not just two commands. **`drizzle-kit generate` exits 0 when it fails.** Measured on the pinned 0.31.10 against a renamed column: it needs an interactive answer, cannot get one because CI has no TTY, prints a stack trace, writes no migration — and exits 0. The same holds for an unresolvable import or a bad path. A step that trusted the exit code would then find nothing to diff and pass green over exactly the drift it exists to catch, so the step asserts the tool's own success line (`No schema changes` or `Your SQL migration file`) instead of its exit status. `< /dev/null` is for the developer running it locally, where a TTY does exist and the prompt would otherwise wait for a person. And `git diff` cannot see an untracked new migration file, so without `git add -AN` the check would rest on drizzle-kit also appending to the tracked `_journal.json` rather than on the migration itself. The step needs no database; `generate` reads the schema files and the snapshots.
