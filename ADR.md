...
Locally both stores run from `docker-compose.yml` (`postgres:16-alpine` and `redis:7-alpine`, healthchecked, on named volumes and a named network; the application containers, the migration step and the `monitoring` and `tools` profiles are added on top of it by issue #19). The application takes its wiring from the environment — `DATABASE_URL`, `REDIS_URL` and the two logical-database indexes `REDIS_READ_MODEL_DB` (read model) and `REDIS_QUEUE_DB` (BullMQ) — documented in `.env.example` and read by `loadConfig` in `src/shared/config.ts`, which is written and tested but not yet called by any process — it derives `redisReadModelUrl` and `redisQueueUrl`, rejects a `DATABASE_URL` or `REDIS_URL` whose scheme or host is wrong, rejects a `DATABASE_URL` carrying no database name, and rejects a configuration that points the read model and the queue at the same logical database. `loadConfig` has no consumer yet, deliberately: the first module that needs a connection wires it in and reconciles the duplicate `PORT` read in `src/server.ts`, so it is pending work rather than dead code. The connection URLs are the only source of truth for where the application connects. The database name is required because node-postgres would otherwise fall back to `PGDATABASE`, then the user name, then the OS user, so an omitted name connects to something and the mistake surfaces later as missing tables. `REDIS_URL` needs no equivalent check: `redisUrlForDb` overwrites the path with the validated index, so a path there decides nothing. `docker-compose.yml` publishes 5432 and 6379 on `127.0.0.1` as literals rather than through `POSTGRES_PORT` and `REDIS_PORT` overrides, and nothing must reintroduce those two variables: they are the names Kubernetes injects for Services called `postgres` and `redis`, in the form `POSTGRES_PORT=tcp://10.96.1.5:5432`, so they are not ours to read. A cross-check against a variable this application does not own adds a failure mode and buys nothing — it briefly did, and with a sidecar proxy making `DATABASE_URL` loopback it would have crash-looped every pod in the namespace at startup, blaming a compose file that deployment never had. A developer whose 5432 is occupied edits the compose file and the URL together.
