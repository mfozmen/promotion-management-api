...
- Deleting the activity guard rather than repointing it at the database clock is a decision, not a cleanup (owner, 2026-09-13, issue #28): the application can no longer ask whether a promotion row it already holds is active, so anything that needs to know re-reads through the gating query and a caller holding a stale row has no local way to detect it. The boundary tests that pinned `starts_at <= now() < ends_at` in TypeScript went with it, and that predicate is now covered only where it runs, in SQL, which this branch has no integration harness for (PR #50). Acceptable because one predicate on one clock is the point of REVIEW.md 2.7: a second implementation that agrees except at the boundary is worse than none, and the boundary is exactly where ADR-0006's delayed jobs are supposed to make the price crisp.
