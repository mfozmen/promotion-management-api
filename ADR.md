...
A third service, `migrate`, runs `npm ci --ignore-scripts && npx drizzle-kit migrate` in a `node:22-alpine` container behind `depends_on: postgres: service_healthy`, with its `DATABASE_URL` composed from the same `POSTGRES_*` variables the `postgres` service reads, so nobody assembles a connection string by hand and the two cannot drift apart in one `.env` (PR #50, `ba5c2ca`; `--ignore-scripts` in `888e632`). It is invoked as `docker compose run --rm migrate` and sits behind a `migrate` profile so a plain `up` does not start it — `run` enables the profile itself.

It was first written as a non-profile service so that `docker compose up -d --wait` alone would leave a migrated schema in place, which is what the owner asked for, and that does not work: **`--wait` has no way to express "this one finished".** A one-shot that exits counts as a wait failure whatever its status, so with the migration inside `up`, a completely healthy stack exits 1 — measured on Compose v5.5.0, `container …-migrate-1 exited (0)` and shell status 1, with `up -d --wait postgres redis` exiting 0 as the control. That is worse than an extra verb twice over: the guide's first command reports failure on success, and a genuine migration failure becomes indistinguishable from the normal case. Neither `up` form carries the migration's status either — attached `up migrate` exits 0 over a migration that exited 17, `up -d --wait` exits 1 — whereas `run --rm migrate` returns 0 and 17 respectively, so it is the only form a script can branch on. These are measured against one Compose version, like the drizzle-kit success-line contract above, and `--wait`'s treatment of exited one-shots has moved across releases. When the `api` image lands (issue #19) the migration moves into its entrypoint and this service goes; the supported way to gate one service on a completed one-shot is `depends_on: migrate: condition: service_completed_successfully`, which is that branch's to add.
...
- Migrations run on their own single-connection pool with both timeouts set to `0`, because a migration killed halfway leaves `__drizzle_migrations` unwritten and every retry replays the same statement. That pool is the test harness's; the production runner is `drizzle-kit migrate`, which reads `drizzle.config.ts` and sets neither, so a managed PostgreSQL with a server-side `idle_in_transaction_session_timeout` needs it lifted for the migration role. `CREATE INDEX CONCURRENTLY` cannot run inside the transactional migrator at all, so a hot-table index (REVIEW.md 11.2) will need a migration applied outside it.
...
- The `migrate` one-shot service runs `npm ci --ignore-scripts` inside its container on every `up`, because `drizzle-kit` pulls in `esbuild` and that binary is platform-specific; the host's `node_modules` is shadowed by an anonymous volume so the Linux install cannot replace a Windows or macOS binary and break the next `npm run dev`. `--ignore-scripts` is what keeps that install from running `prepare` as root in the bind-mounted checkout and leaving a root-owned `.husky/_` behind on a Linux host; esbuild survives it because its binary arrives as the `@esbuild/linux-x64` optional dependency rather than from a postinstall download, verified by transforming TypeScript in that container. The cost is a slow first `up` (a full install) and a second dependency tree to keep in the cache. Accepted as a stopgap: issue #19 builds the `api` image, the migration moves into its entrypoint, and this service goes — the two must never both run migrations.
