...
Which candidate applies is decided by `json-rules-engine` rules stored in `pricing_rules` (`type = 'promotion'`) and cached for 60 seconds, not by hard-coded precedence (owner decision, 2026-09-12). The resolver computes each candidate's discount first, then hands the engine a single fact object holding both candidates, so a rule can compare them; the highest-priority rule that fires names the winning level, ties breaking on the lower promotion id; the seeded default rule is product-level precedence, so a product's own promotion wins over its category's even when the category discount is larger. That default is not free to choose: REVIEW.md 7.4 makes it a required edge case and section 3 of the design spec states it with its consequence for admins. Its opposite, whichever price is lower, is the same policy as "precedence by larger discount" under another name, which is why one of them sat in this ADR's rejected alternatives while its twin sat in the seeded default — that is how the contradiction survived. Changing the policy remains a row edit rather than a deploy.
...
- Under the seeded default a category flash sale skips products that carry their own active promotion, because product level wins. The applied promotion is returned in every storefront response, so an admin can see which candidate won. A product that must not be discounted further is expressed as a higher-priority rule, not as code.
- The retired shape is unstorable as of this branch. Migration `0000_write_store.sql` creates `promotion_discount_type` as an enum of `percentage | fixed` and a `value integer` bounded by `check (value > 0)` and `check (discount_type <> 'percentage' or value <= 10000)`, with no `calculator` or `params` column, so a row naming a calculator class cannot be written. The matching TypeScript half is not on this branch: `src/modules/promotion/promotion.ts` (`DiscountType` as `percentage | fixed`, a `Promotion` interface with no `calculator` field and no parameter bag) and `effective-price.ts` (`applyPromotion` branching on the two literal types, with no registry to look one up in) live in PR #29, open and unmerged. Until it lands, only the storage half is proven here.
