...
**Status:** Accepted — issue #6, PR #30 (branch `feat/http-skeleton`, commit `7fd588e` plus the review-fix commits `e46f14b`, `c6e4ff2`, `ccef8a4`, `de3bf9e`, `af38e0c`, `4f10c7f`, `a218bf2`, `9f27f8d`, `5f4da57`, `ec623cc`, `30bc002`, `f621699`, `4ec8a35`, `f4a27c0` and `acc81ae`)
...
- **The tables the envelope answers from are frozen through their rows.** `CLIENT_ERRORS`, `SERVER_FAULT` and `SERVER_MESSAGES` are spread into a response body, so a shallow freeze — which holds the key-to-row binding and leaves every row writable — would let one assignment from any consumer rewrite the body of every `413` in the process. Each row is frozen individually, and the retriable list is a frozen array rather than a `ReadonlySet`, whose `.add` a freeze does not stop and whose readonly type is erased at build time (commit `f4a27c0`).
- **Both error bounds are one constant each, in `src/shared/error-bounds.ts`.** `MAX_DETAILS` is read by the validator, which bounds the work of building the list, and by the envelope, which bounds what is returned; `MAX_MESSAGE` is read by the envelope and by the logger. Each had two copies kept in step by a comment claiming they matched, which is a coupling nobody maintains. What belongs in that module is a bound with more than one enforcement point (commits `acc81ae`, `6b4203c`).
...
**Status:** Accepted — issue #6, PR #30 (branch `feat/http-skeleton`, commit `7fd588e` plus the review-fix commits `e46f14b`, `c6e4ff2`, `ccef8a4`, `87b0a49`, `f621699` and `acc81ae`)
