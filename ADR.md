...
Locally both stores run from `docker-compose.yml` (`postgres:16-alpine` and `redis:7-alpine`, healthchecked, on named volumes and a named network; the application containers, the migration step and the `monitoring` and `tools` profiles are added on top of it by issue #19). The application takes its wiring from the environment — `DATABASE_URL`, `REDIS_URL` and the two logical-database indexes `REDIS_READ_MODEL_DB` (read model) and `REDIS_QUEUE_DB` (BullMQ) — documented in `.env.example` and read once at startup by `loadConfig` in `src/shared/config.ts`, which derives `redisReadModelUrl` and `redisQueueUrl`, rejects a `DATABASE_URL` or `REDIS_URL` whose scheme or host is wrong, rejects a configuration that points the read model and the queue at the same logical database, and rejects a configuration whose scheme or host is wrong. The connection URLs are the only source of truth for where the application connects. `docker-compose.yml` publishes 5432 and 6379 on `127.0.0.1` as literals rather than through `POSTGRES_PORT` and `REDIS_PORT` overrides, and nothing must reintroduce those two variables: they are the names Kubernetes injects for Services called `postgres` and `redis`, in the form `POSTGRES_PORT=tcp://10.96.1.5:5432`, so they are not ours to read. A cross-check against a variable this application does not own adds a failure mode and buys nothing — it briefly did, and with a sidecar proxy making `DATABASE_URL` loopback it would have crash-looped every pod in the namespace at startup, blaming a compose file that deployment never had. A developer whose 5432 is occupied edits the compose file and the URL together.

The PostgreSQL healthcheck runs an authenticated `psql` query rather than `pg_isready`, which only sends a startup packet: it reports "accepting connections" (exit 0) for a database that does not exist, so a password or database name changed in `.env` against an already-initialised volume reported healthy while every `depends_on: service_healthy` dependant started into a crash loop. `psql` authenticates over the container's unix socket and reads the credentials from the container's own environment, so nothing secret reaches the process list.
