...
- **The boundary is one directory named for what it does.** The whole HTTP boundary — the error class, its code vocabulary and status table, the two handlers, the validator and the logger — lives in `src/shared/http/`, with its tests mirrored at `tests/unit/shared/http/`. It was `src/middleware/`, which named a kind of Express syntax rather than a role and could not hold the logger or the error class without lying; REVIEW.md 8c.7 is the rule, and the move is commit `459686b`. Inside it every file holds one exported declaration (8c.2), so `http-error.ts` is the class alone and `error-code.ts`, `status-by-code.ts`, `client-errors.ts`, `other-client-error.ts`, `max-details.ts`, `max-message.ts`, `validation-detail.ts` and `not-found-handler.ts` each stand on their own (commits `a624f6a`, `a2c8a1a`). Trade-off: fourteen small files where three would do, and an import list at the top of the handler that is longer than the rule it enforces. Accepted because each of these is a boundary contract other modules will import — the code set, the status table, the two bounds — and a grep for one of them lands on its definition rather than on a grab-bag module.
