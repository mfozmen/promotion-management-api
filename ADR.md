...
**Status:** Accepted — issue #6, PR #30 (branch `feat/http-skeleton`, commit `7fd588e` plus the review-fix commits `e46f14b`, `c6e4ff2`, `ccef8a4`, `de3bf9e` and `af38e0c`)
...
- **A 5xx keeps its status; its code and its words have to be earned.** A code whose status does not match it is worse than no code: a client branching on `CONFLICT` under a `500` would never retry what is actually a server fault. A status Express cannot write collapses to `500`; a real 5xx keeps its status, because `502` and `504` are exactly what an operator needs and collapsing them to `500` deletes the signal. The code and the message cross only where we wrote public words for that code, from a map keyed by `ErrorCode` — ours by construction rather than by trusting the raiser, so `503 READ_MODEL_NOT_READY` reads "The read model is not ready yet; retry shortly" rather than "Internal server error", which would tell a human the opposite of the truth. A handler's `details` never crosses on a 5xx either. The generic `"Internal server error"` is the fallback for a 5xx code with no entry in the map (commit `af38e0c`).
- **The rejected keys are logged at `warn`, not `debug`.** The root logger runs at pino's default `info`, so a `debug` line is a mitigation that never fires in production while passing every test — which is what the first version of this did. Field names only, never values, and capped, because the number of unknown keys is the client's to choose. An `HttpError` message must therefore never carry a credential: on a 5xx it goes to the log and not to the client, but it does go to the log. `tests/request-validator.test.ts` builds the logger at `info` so the mitigation cannot silently stop firing again (commit `af38e0c`).
- **A rejection names where, never what.** "Client input is not echoed back" now holds on all three paths, not two: `notFoundHandler` withholds the path, body-parser's messages are replaced, and zod's unrecognized-key message — which quotes the key the client sent — is replaced with the location and a count. The path (`body`, `body.window`, `body.items[3].sku`) is ours, we generate it, and the client needs it to fix the request; the key and the value are theirs. The rejected keys go to the log at `warn` with the correlation id (see the bullet above for why not `debug`), so an operator can still answer "what did they send".
...
- Peak RSS under load is a V8 heap question, not an application one: `e2e-tester` measured 209 MB at `-c 100` on the built artifact for a route that serialises `{"status":"ok"}`, returning to 59 MB afterwards. On a 256 MB container that is thin headroom before any database or Redis client exists, so the deployment sets `--max-old-space-size` below the container limit rather than trusting the default.
