...
- Resolution: `src/server.ts` now stamps `Date.now()` before the wait and logs the elapsed difference, and parses the override with `Number.isFinite` so `0` is honoured and only an absent or unparseable value falls back to the default.
