...
- The round became two rules: `main`'s 80323ae added `REVIEW.md` 8b.6 (configuration files carry no narrative; `.env.example` at most one short line per variable) and 12.5 (startup validation covers only what fails late and quietly — one zod parse, one refine per invariant), the second citing this pull request as its evidence. `main` was merged before the final push so the branch is judged by the rules it produced. `docker-compose.yml` has no comments; `.env.example` is 10 short lines for 15 variables, each one an invariant or a coupling the entry cannot state; `src/shared/config.ts` is 37 lines with a refine per invariant, which is the shape 12.5 describes. The `.env.example` file was parsed through the schema to prove the trim did not break the example it documents.
