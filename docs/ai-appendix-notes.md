...
- Mid-flight design change absorbed rather than re-litigated: while the branch was open the owner ruled that a promotion names the class that prices it, so `discount_type` and `value` became `calculator` and `params` (PR #35). The branch was rebased onto that revision instead of landing a schema the spec no longer described, and the constraint test for the percentage ceiling was replaced by one asserting the opposite — an unknown calculator name is accepted, because validating it belongs to the registry and not to a column.
