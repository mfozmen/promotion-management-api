...
- Open after the merge, not a docs item: `.claude/agents/e2e-tester.md` resolved to `main`'s rewrite, which restored the pre-`/api` load targets (`GET /products/:id`, `GET /products?category=…`) that this branch had corrected. Every route is mounted under `/api` (ADR-0008), so those two steps and the matching pass criterion would 404 on the first run. Named here because it is a code-side fix, not a document one.
