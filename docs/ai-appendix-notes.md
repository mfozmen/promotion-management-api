...
### 2026-09-12 — A rule patched at the wrong width, and a comment that described a control it did not have (issue #6, PR #30, commits `4ec8a35` and `f4a27c0`)
...
### 2026-09-12 — The rule was widened and broken one commit apart (issue #6, PR #30, commit `acc81ae`)

- Challenge: the advisory review found that the test added alongside the 5xx logging fix passed its cause as `HttpError`'s third argument, which is `details`, not `Error.cause`. `serializeError` reads `err.cause`, so the injected `ECONNREFUSED` error was inert: the test pinned the two new top-level fields and never the claim its own comment made, and it would have passed identically with the cause deleted. That is REVIEW.md 7.4b — assert the reachable breach, not the one the control obviously covers. The rule itself came from `main` (commit `731c6df`, PR #46); what this branch did, in the commit immediately before this one (`f4a27c0`), was give 7.4b its third piece of evidence and write that closing sentence. So the sentence broken here is one this branch had authored one commit earlier, under a rule it did not write. The first draft of this entry said the pull request had added the rule itself two commits earlier; the commit order refutes both halves, and the claim is corrected here rather than left standing.
- Verification: the corrected test sets `raised.cause` and asserts the serialized error carries `ECONNREFUSED` while the line carries the 503 the client read, so both halves of the sentence are now held by an assertion. Then the fix was removed on purpose to watch the test fail, and restored — the check the original round skipped.
- Resolution: the test rewritten, the stale `AppError(status, code, message)` signature corrected in `docs/e2e-cases/6.md` (the class became `HttpError(code, message, details?)` several rounds earlier, and prose outliving a rename is the third time on this branch), and `MAX_DETAILS` moved into one module rather than declared twice and kept in step by a comment.
- Lesson: restating a rule does not inoculate you against it. Its evidence had been widened by this branch one commit earlier, and the next test still asserted the comfortable half. What catches this is not knowing the rule but the mechanical habit of breaking the production code to see the test go red.
