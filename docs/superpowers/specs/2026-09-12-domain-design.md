...
| `maintenance` | `readmodel.rebuild` | `{ category?: string }`              | cold start, admin, reconciler                                               | `SCAN`+`UNLINK` the scope, stream products from PostgreSQL, rebuild; full rebuild sets `readmodel:ready`              |
| `maintenance` | `reconcile.run`     | `{}` (repeatable, every 5 min)       | reconciler worker schedule                                                  | see section 9                                                                                                         |
