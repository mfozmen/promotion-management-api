...
| `readmodel:source-read-at` | HASH | field = product id, value = `<epoch microseconds>:<category>` of the last write that applied — the ordering token of ADR-0003, kept after a removal so absence means never written, and the category so the next write clears the membership the product leaves                                        |
