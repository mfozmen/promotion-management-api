...
const READ_AT = '2026-09-14T10:00:00.000000+00:00';
...
function collaborators(rows: ReturnType<typeof row>[]) {
  ...
  const stored = {
    find: vi.fn().mockResolvedValue({ category: 'knitwear' }),
  } as unknown as ProductReadRepository;

  return { source, write, stored };
}

describe('ProductUpsertedHandler', () => {
  it('writes every product the announcement named', async () => {
    const { source, write, stored } = collaborators([row(1), row(2)]);

    await new ProductUpsertedHandler(source, write, stored).handle({ productIds: [1, 2] });
  ...
  it('writes with the instant PostgreSQL read, not one the worker made up', async () => {
    const { source, write, stored } = collaborators([row(1)]);

    await new ProductUpsertedHandler(source, write, stored).handle({ productIds: [1] });
  ...
  it('prices a product at its base until a promotion resolver exists', async () => {
    const { source, write, stored } = collaborators([row(1, { basePriceCents: 7_000 })]);
  ...
  it('removes an id the announcement named and PostgreSQL no longer holds', async () => {
    ...
    // The category comes from the entry that is about to go: a deleted row
    // cannot say which sorted set it was scored in.
    expect(vi.mocked(write.remove).mock.calls[0]?.[1]).toBe('knitwear');
  });

  it('omits the pricing rules version when the row carries none', async () => {
    const { source, write, stored } = collaborators([row(1, { pricingRulesVersion: null })]);
  ...
  it('removes nothing when the entry is already gone', async () => {
    const { source, write, stored } = collaborators([]);
    vi.mocked(stored.find).mockResolvedValue(undefined);

    await new ProductUpsertedHandler(source, write, stored).handle({ productIds: [1] });

    // Two announcements for one delete is ordinary on an at-least-once queue,
    // and the second has nothing to clear.
    expect(write.remove).not.toHaveBeenCalled();
  });

  it('reads once for the whole batch rather than once per product', async () => {
    const { source, write, stored } = collaborators([row(1), row(2), row(3)]);

    await new ProductUpsertedHandler(source, write, stored).handle({ productIds: [1, 2, 3] });
  ...
  it('does not fail the batch when a write is refused by an older token', async () => {
    ...
    await expect(
      new ProductUpsertedHandler(source, write, stored).handle({ productIds: [1, 2] }),
