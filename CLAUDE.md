...
- **Before every push**, run the agents the change actually needs, and only those. `docs-scribe` always. `e2e-tester` when the diff touches something that can change how the running application behaves: `src/`, `tests/`, `package.json`, a `*.config.ts`, a `tsconfig*.json`, a `Dockerfile` or a compose file. `impact-analyzer` for those same paths and for `.github/workflows/`, because a change to how pull requests are judged lands on every branch at once. `architecture-critic` when the PR touches `ADR.md`, `docs/superpowers/specs/`, `src/modules/vendor/`, `src/modules/promotion/`, `src/modules/pricing/`, `src/workers/`, or carries the `scenario` label. A documentation or rulebook edit cannot leak a connection or deadlock a transaction, so it does not wait for a load run. Push only on PASS/SOUND with the docs changes committed, then apply the matching labels; `local-gates` computes the same set from the changed paths and strips every label on each new push.
