...
- Address Claude review comments before asking for the owner's check; resolve review threads once handled. Wait for the review run to finish, then fix all findings in one commit; only Critical findings and REVIEW.md blocking rules must be fixed in the PR, Warnings may be deferred to a linked issue, Suggestions are answered.
