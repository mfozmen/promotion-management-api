...
Instructions for Claude Code sessions working in this repository. For the stack and how to run it, read `package.json` (dependencies and scripts), `tsconfig.json` (compiler settings), `vitest.config.ts` and `vitest.workspace.ts` (test layers and coverage settings) and `.github/workflows/` (CI).
